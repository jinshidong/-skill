...
- ✅ 数学公式和 Mermaid 图表自动适配主题背景色（确保对比度）
- ✅ 多种主题风格（学术灰、节日、科技、公告）
- ✅ 可扩展的风格配置系统
- ✅ 支持列表（有序/无序，支持嵌套）
- ✅ 支持表格（支持对齐方式：左、中、右）
...
- **主题适配**：
  - 自动根据主题背景色调整公式背景色，确保对比度
  - 如果主题背景是纯白色或接近白色（RGB > 240），使用透明背景
  - 其他情况下使用白色背景，确保公式在任何背景下都清晰可见
- **行内公式优化**：
...
- **自动清理**：转换完成后会自动删除临时文件
- **主题适配**：
  - 自动根据主题背景色调整图表背景色，确保对比度
  - 如果主题背景是纯白色或接近白色（RGB > 240），使用透明背景
  - 其他情况下使用白色背景，确保图表在任何背景下都清晰可见
