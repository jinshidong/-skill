...
**替代方案**：
- 对于简单的数学符号或变量，使用纯语言描述替代（如"碰撞参数"、"光程模型"等）
...
- **背景强调**：
  ...
  - 添加大量噪声（零宽字符、随机属性、HTML注释等）以避免被过滤
  - 图片噪声增强（高斯噪声、随机像素点、边缘噪声等）
- **公式使用限制**：
  - ⚠️ **绝对不要使用行内公式（`$...$`）**：即使添加了噪声和背景色，微信公众号平台仍会过滤掉所有行内公式
  - ⚠️ **绝对不要在列表中使用块级公式（`$$...$$`）**：会导致格式混乱，影响列表的编号和层级结构
  - **推荐做法**：所有公式使用块级公式（`$$...$$`），在段落中独立显示；列表中的公式转换为纯语言描述

**⚠️ 重要提示：微信公众号公式过滤问题**

- **行内公式（`$...$`）**：⚠️ **绝对禁止使用**
  - **严格限制**：微信公众号平台会过滤掉所有行内公式（小图片），即使程序已添加噪声和背景色，仍会被移除
  - **替代方案**：
    - 使用块级公式（`$$...$$`）在段落中独立显示
    - 使用纯语言描述替代简单的数学符号或变量
- **块级公式（`$$...$$`）**：**推荐使用，但有位置限制**
...
4. **数学公式**：本地渲染为图片并转为 base64 嵌入，仅支持块级公式（`$$...$$`）
   - ⚠️ **限制**：绝对不要使用行内公式（`$...$`），绝对不要在列表中使用块级公式
