...
⚠️ **内联公式在微信公众号中会被过滤，需要不用或慎用**

- **当前状态**：程序可以正确识别并精准输出内联公式（`$...$`）
- **微信公众号限制**：微信公众号平台会过滤掉内联公式（小图片）
- **使用建议**：
  - **优先使用块级公式**：对于简单公式，使用 `$$...$$` 块级公式更安全
  - **复杂内联公式**：对于复杂的内联公式，可能不会被过滤（但不确定），可谨慎使用
  - **注意**：简单的块状公式也有被过滤的现象，需要根据实际情况测试和调整
  - **解决方案**：程序已为内联公式添加米黄色背景和噪声，但仍可能被过滤，建议优先使用块级公式
...
- 代码块（支持语法高亮）
- 数学公式（行内 `$...$` 和块级 `$$...$$`）
  - ⚠️ **重要限制**：**内联公式在微信公众号中会被过滤，需要不用或慎用**
    - 当前程序可以识别并精准输出内联公式
    - 但微信公众号平台会过滤掉内联公式（小图片）
    - **建议**：
      - 对于简单公式，优先使用块级公式 `$$...$$`
      - 对于复杂的内联公式，可能不会被过滤（但不确定），可谨慎使用
      - 注意：简单的块状公式也有被过滤的现象，需要根据实际情况调整
- Mermaid 图表
- 水平分割线（`---`, `***`, `___`）
- 粗体、斜体、行内代码等内联格式
